        ...
        bind: function (videoEl) {
            ...
            videoEl.addEventListener('timeupdate', function () {
                ...
                if (duration > 10) {
                    for (var i = 0; i < sectionArr.length; i++) {
                        if (sectionArr[i] === 0
                        && ((self.currentTime / duration) >= (i / sectionArr.length))
                        && ((self.currentTime / duration) < ((i + 1) / sectionArr.length))) {
                            sectionArr[i] = 1;
                            // Data of 0% is replace by play times
                            if (i !== 0) {
                                videoLog.ext.cent = (i / sectionNum) * 100 + '%'
                                send('section', loc, network);
                            }
                        }
                    }
                }
            ...
            videoEl.addEventListener('ended', function () {
                videoLog.ext.cent = '100%';
                sendLog('section', loc, network);
            });
            // Error log
            videoEl.addEventListener('error', function () {
                var data = videoEl.error;
                sendLog('error', loc, network, data);
            });
        },
        getNetwork: function () {
            var self = this;
